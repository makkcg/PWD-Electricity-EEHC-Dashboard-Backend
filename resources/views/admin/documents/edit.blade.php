...
<section class="content clearfix">
    <form method="post" action="{{ route('admin.documents.update',$item->id) }}" enctype="multipart/form-data">
     @csrf
        @method("put")
 <h4 class="pull-left">مستند
  <small>تحديث مستند</small>
</h4>

<div class="pull-right">
  <div class="col-md-12 text-right">

    <button type="submit" class="btn btn-primary margin-r-5" ><i class="fa fa-save margin-r-5"></i>تحديث مستند</button>
...
<script type="text/javascript">
  $(document).ready(function() {
      $('.deleteImage').click(function() {
          ...
          var url = '{{ route("document.imageAjax", ":imageID") }}';
...
<script type="text/javascript">
  $(document).ready(function() {
      $('.deleteTitleSound').click(function() {
          ...
          var url = '{{ route("document.titleSoundAjax", ":soundID") }}';
...
<script type="text/javascript">
  $(document).ready(function() {
      $('.deleteDescSound').click(function() {
          var soundID = $(this).val();
          var url = '{{ route("document.descSoundAjax", ":soundID") }}';
            url = url.replace(':soundID',soundID);
                $.ajax({
                    url: url,
                  type: "GET",
...
<script type="text/javascript">
  $(document).ready(function() {
      $('.deleteTitleVideo').click(function() {
          ...
          var url = '{{ route("document.titleVideoAjax", ":videoID") }}';
...
$(document).ready(function() {
    $('.deleteDescVideo').click(function() {
        var videoID = $(this).val();
        var url = '{{ route("document.descVideoAjax", ":videoID") }}';
            url = url.replace(':videoID',videoID);
                $.ajax({
                    url: url,
                type: "GET",
