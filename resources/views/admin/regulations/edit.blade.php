...
<section class="content clearfix">
    <form method="post" action="{{ route('admin.regulations.update',$item->id) }}" enctype="multipart/form-data">
     @csrf
        @method("put")
 <h4 class="pull-left">الشروط والاحكام
  <small>تحديث الشروط والاحكام</small>
</h4>

<div class="pull-right">
  <div class="col-md-12 text-right">

    <button type="submit" class="btn btn-primary margin-r-5" ><i class="fa fa-save margin-r-5"></i>تحديث الشروط والاحكام</button>
  ...
  <div class="row">
    <div class="col-xs-12 row">
     ...
     @include('admin.regulations.form')
...
<script type="text/javascript">
  $(document).ready(function() {
      $('.deleteImage').click(function() {
          ...
          var url = '{{ route("regulation.imageAjax", ":imageID") }}';
...
<script type="text/javascript">
  $(document).ready(function() {
      $('.deleteTitleSound').click(function() {
          ...
          var url = '{{ route("regulation.imageAjax", ":soundID") }}';
...
<script type="text/javascript">
  $(document).ready(function() {
      $('.deleteDescSound').click(function() {
          var soundID = $(this).val();
          var url = '{{ route("regulation.descSoundAjax", ":soundID") }}';
            url = url.replace(':soundID',soundID);
                $.ajax({
                 url: url,
                  type: "GET",
  ...
  $(document).ready(function() {
      $('.deleteTitleVideo').click(function() {
          var videoID = $(this).val();
              $.ajax({
                  url: '/admin/regulations/title_video/ajax/'+videoID,
                  type: "GET",
                  dataType: "json",
                  success:function(data) {
                      console.log('deleted');
...
$(document).ready(function() {
    $('.deleteDescVideo').click(function() {
        var videoID = $(this).val();
        var url = '{{ route("regulation.descVideoAjax", ":videoID") }}';
