...
<section class="content clearfix">
    <form method="post" action="{{ route('admin.procedures.update',$item->id) }}" enctype="multipart/form-data">
     @csrf
        @method("put")
 <h4 class="pull-left">اجراء
  <small>تحديث اجراء</small>
</h4>

<div class="pull-right">
  <div class="col-md-12 text-right">

    <button type="submit" class="btn btn-primary margin-r-5" ><i class="fa fa-save margin-r-5"></i>تحديث اجراء</button>
...
<script type="text/javascript">
  $(document).ready(function() {
      $('.deleteImage').click(function() {
          ...
          var url = '{{ route("procedure.imageAjax", ":imageID") }}';
...
<script type="text/javascript">
  $(document).ready(function() {
      $('.deleteTitleSound').click(function() {
          ...
          var url = '{{ route("procedure.imageAjax", ":soundID") }}';
...
<script type="text/javascript">
  $(document).ready(function() {
      $('.deleteDescSound').click(function() {
          var soundID = $(this).val();
          var url = '{{ route("procedure.descSoundAjax", ":soundID") }}';
            url = url.replace(':soundID',soundID);
                $.ajax({
                    url: url,
                  type: "GET",
  ...
  $(document).ready(function() {
      $('.deleteTitleVideo').click(function() {
          var videoID = $(this).val();
              $.ajax({
                  url: '/admin/procedures/title_video/ajax/'+videoID,
                  type: "GET",
                  dataType: "json",
                  success:function(data) {
                      console.log('deleted');
...
$(document).ready(function() {
    $('.deleteDescVideo').click(function() {
        var videoID = $(this).val();
        var url = '{{ route("procedure.descVideoAjax", ":videoID") }}';
